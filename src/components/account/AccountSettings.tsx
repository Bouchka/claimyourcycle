...
export function AccountSettings() {
  ...
  const handleSignOut = async () => {
    try {
      await signOut(auth);
      navigate('/sign-in'); // Ensure this redirects to sign-in page
    } catch (error) {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      navigate('/sign-in', { replace: true }); // Redirect to sign-in after deletion
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-2xl mx-auto p-6">
        <h1 className="text-2xl font-serif text-primary mb-8">
          Account Settings
        </h1>
